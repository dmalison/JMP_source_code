...
//  vector[N] epsilon_R_2;
  vector[N] epsilon_N_2;
//  vector[N] epsilon_C_2;
...
/*** relationship indicators ***/
  ...
  // vector[3] gamma_p_; // (extra undercore at end because gamma_p is a protected function)
...
//    lambda[R_2_ind0,2:3] * c_NC_diag +
  ...
  theta_N_2[R_2_ind_nomiss] =
    X_Q_nocons[R_2_ind_nomiss,]*alpha_2_tilde_raw[,2] +
    epsilon_N_2[R_2_ind_nomiss];

  // theta_C_2[R_2_ind_nomiss] = 
  //   X_Q_nocons[R_2_ind_nomiss,]*alpha_2_tilde_raw[,3] + 
  //   epsilon_C_2[R_2_ind_nomiss];
...
//  theta_2_mean[3] = mean(theta_NC_2[R_2_ind_nomiss,2]);
  theta_2_mean[2] = mean(theta_N_2[R_2_ind_nomiss]);
//  theta_2_mean[3] = mean(theta_C_2[R_2_ind_nomiss]);

//  theta_2_sd[1] = sd(theta_R_2);
//  theta_2_sd[2] = sd(theta_NC_2[R_2_ind_nomiss,1]);
//  theta_2_sd[3] = sd(theta_NC_2[R_2_ind_nomiss,2]);
  theta_2_sd[2] = sd(theta_N_2[R_2_ind_nomiss]);
//  theta_2_sd[3] = sd(theta_C_2[R_2_ind_nomiss]);
...
//  theta_2[R_2_ind_nomiss,3] = (theta_NC_2[R_2_ind_nomiss,2] - theta_2_mean[3])/theta_2_sd[3];
  theta_2[R_2_ind_nomiss,2] = (theta_N_2[R_2_ind_nomiss] - theta_2_mean[2])/theta_2_sd[2];
//  theta_2[R_2_ind_nomiss,3] = (theta_C_2[R_2_ind_nomiss] - theta_2_mean[3])/theta_2_sd[3];
...
//  epsilon_C_2 ~ normal(0,1);
  epsilon_N_2 ~ normal(0,1);
