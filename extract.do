...
cm1edu cm1bsex cm1ethrace cm1age cm1relf cm1hhinc cm1adult cm1kids /// pre-constructed variables (see documentation)
m1natwt ///
...
merge 1:1 idnum using "~/data/Fragile_Families/coremerged/ff_pub_merge2.dta", nogen ///
	keepusing( ///
	cf1edu cf1ethrace cf1age cf1hhinc cf1adult cf1kids /// pre-constructed variables (see documentation)
	f1a4 f1a6 f1a6a ///
...
* How long parents knew each other before pregnancy (in years)
	...
	* Trim if report knowing each other before age 5
...
* M has children with other father
	...
	gen M_oth_chd_wo_F = 0 if m1a12 == 2 // mark as zero if no other children
	replace M_oth_chd_wo_F = 1 if m1a12a > f1a6a & m1a12a >= 0 & f1a6a >=0 // mark as one if more children than father
	
* F has children with other mother

	gen F_oth_chd_wo_M = 0 if f1a6 == 2 // mark as zero if no other children
	replace F_oth_chd_wo_M = 1 if m1a12a < f1a6a & m1a12a >= 0 & f1a6a >=0 // mark as one if more children than mother

}
*** GENERATE BASELINE RELATIONSHIP STATUS ***
{
*** Rules about constructing romantic involvement variables ***

*   - relationship status is binary (either romantically involved or not)
*   - separation is an absorbing state, so R_{t} = 0 if R_{t-1} = 0
...
*** GENERATE FIRST YEAR RELATIONSHIP STATUS ***
{
...
*** GENERATE FIRST YEAR NON-COGNITIVE ABILITY MEASUREMENTS ***
{
*** Five-category measurements ***
...
*** GENERATE THIRD YEAR RELATIONSHIP STATUS ***
{
...
*** GENERATE THIRD YEAR RELATIONSHIP QUALITY MEASUREMENTS *** 
{
*** Five-category measurements *** 
...
*** GENERATE THIRD YEAR NON-COGNITIVE ABILITY MEASUREMENTS ***
{
local vars ///
...
*** GENERATE THIRD YEAR COGNITIVE ABILITY MEASUREMENTS ***
{
...
{
local vars2 `vars1' M_R_2_* M_N_2_* M_C_2_* theta_R_2 theta_N_2 theta_C_2 returner social_f
keep `vars2'
}
*** MERGE FIFTH YEAR FOLLOW UP ***
{
merge 1:1 idnum using "~/data/Fragile_Families/coremerged/ff_pub_merge2.dta", nogen ///
	 keepusing( ///
	 m4intmon m4intyr ///
...
*** GENERATE FIFTH YEAR RELATIONSHIP QUALITY MEASUREMENTS *** 
{
*** Five-category measurements ***
...
*** GENERATE FIFTH YEAR NON-COGNITIVE MEASUREMENTS ***
{
...
*** GENERATE FIFTH YEAR COGNITIVE ABILITY MEASUREMENTS ***
{
...
*** KEEP FIFTH YEAR VARIABLES ***
{
local vars3 `vars2' M_R_3_* M_N_3_* M_C_3_* theta_R_3 theta_N_3 theta_C_3 returner social_f
keep `vars3'
}
*** MERGE NINTH YEAR FOLLOW UP ***
{
...
*** GENERATE NINTH YEAR RELATIONSHIP STATUS ***
{
...
*** GENERATE NINTH YEAR RELATIONSHIP QUALITY MEASUREMENTS ***
{
*** Five-category measurements ***
...
*** GENERATE NINTH YEAR NON-COGNITIVE MEASUREMENTS ***
{
...
*** GENERATE NINTH YEAR COGNITIVE MEASUREMENTS ***
{
...
*** GENERATE NINTH YEAR OUTCOME ANCHORS ***
{
gen belowavg_l_yr9 = (t5c13a <= 2) if t5c13a >=0 & t5c13a < .
gen belowavg_s_yr9 = (t5c13b <= 2) if t5c13b >=0 & t5c13b < .
gen belowavg_m_yr9 = (t5c13c <= 2) if t5c13c >=0 & t5c13c < .
gen specialed_yr9  = (t5c1 == 1) if t5c1 <= 2 & t5c1 >= 0
gen bip_yr9        = (t5c7p == 1) if t5c7p >= 0 & t5c7p < .
gen ADHD_yr9       = (t5c6 == 1) if t5c6 >= 0 & t5c6 < .
gen damage_yr9 = (k5f1a == 1) if k5f1a <= 2 & k5f1a >= 0
...
foreach var of varlist belowavg_l_yr9-suspend_yr9{

	gen anchor_`i_anchor' = .
	
	replace anchor_`i_anchor' = `var' if `var' >= 0 & `var' < .

	local i_anchor = `i_anchor' + 1

}
}
*** KEEP NINTH YEAR VARIABLES ***
{

local vars4 `vars3' M_R_4_* M_N_4_* M_C_4_* theta_R_4 theta_N_4 theta_C_4 anchor* returner social_f
keep `vars4'
...
replace R_0 = 1 if R_0 == . & (R_1 == 1 | R_2 == 1 | R_3 == 1 | R_4 == 1)
replace R_1 = 1 if R_1 == . & (R_2 == 1 | R_3 == 1 | R_4 == 1)
replace R_2 = 1 if R_2 == . & (R_3 == 1 | R_4 == 1)
replace R_3 = 1 if R_3 == . & (R_4 == 1)

* assume parents are together until first reported separation

replace R_0 = 1 if R_0 == . & (R_1 != . | R_2 != . | R_3 != . | R_4 != .)
replace R_1 = 1 if R_1 == . & (R_2 != . | R_3 != . | R_4 != .)
replace R_2 = 1 if R_2 == . & (R_3 != . | R_4 != .)
replace R_3 = 1 if R_3 == . & (R_4 != .)
...
foreach var of varlist M_R_4_* M_N_4_* M_C_4_* anchor*{
