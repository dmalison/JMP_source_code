...
M_sim <- function(variable, period, n_cat){
  ...
  if (variable == "C"){
    ...
    mu_M <- get(paste("mu_","M_",  name, sep = ""))
    sigma_M <- get(paste("sigma_","M_",  name, sep = ""))
  ...
  theta <- get(paste("theta", period, sep = "_"))
...
{
  for (i in (1:(nrow(M_frame)))[-c(8,12,16)]){
...
}
{
  ...
  mu_M_C_3_mean <- stan_data$mu_M_C_3_mean
  gamma_M_C_3_2_mean <- stan_data$gamma_M_C_3_2_mean
  sigma_M_C_3_mean <- stan_data$sigma_M_C_3_mean
